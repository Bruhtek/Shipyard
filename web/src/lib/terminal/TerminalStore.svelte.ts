...
class terminalStore {
	...
	subscriptionsFinishes: (() => void)[] = [];

	subscribeActionFinished(callback: () => void) {
		this.subscriptionsFinishes.push(callback);
	}
	unsubscribeActionFinished(callback: () => void) {
		this.subscriptionsFinishes = this.subscriptionsFinishes.filter((cb) => cb !== callback);
	}
	...
	addOrUpdateTerminal(metadata: ActionMetadata) {
		...
		if (
			this.subscriptionsFinishes.length > 0 &&
			(t.Status == ActionStatus.Success || t.Status == ActionStatus.Failed)
		) {
			this.subscriptionsFinishes.forEach((cb) => cb());
		}
	}
	...
	filterTerminalsById(ids: string[]) {
		...
		const filteredSubscriptions = subscriptionIds.filter(
			(id) => !ids.includes(id) && !this.terminals.find((t) => t.id === id)
		);
