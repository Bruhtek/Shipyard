...
func Handler(message []byte) {
	...
	env := environments.EnvManager.GetEnv(envName)
	if env == nil {
		log.Error().
	...
	if env.GetEnvType() == "remote" {
		HandleRemoteAction(env.(types.RemoteEnvironment), objectType, action, objectId)
		return
	}
...
func HandleRemoteAction(env types.RemoteEnvironment, objectType string, action string, objectId string) {
