...
func (r *RemoteEnvironment) GetResponse(path string) (types.RequestResponse, error) {
	r.Need()
	...
	if !connected {
		...
		return types.RequestResponse{}, errors.New("connection timed out")
	}
	...
	if err != nil {
		return types.RequestResponse{}, err
	}
...
func (r *RemoteEnvironment) PostResponse(path string, body string) (types.RequestResponse, error) {
	r.Need()
	connected := r.waitForConnection()
	if !connected {
		log.Warn().
...
func parseResponseData(data string) (res types.RequestResponse) {
	defer func() {
